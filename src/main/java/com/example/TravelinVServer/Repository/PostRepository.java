...
public interface PostRepository extends JpaRepository<Post, Integer> {
        ...
        @Query("SELECT new com.example.TravelinVServer.Responese.RelatedPostResponse (p.id_post, p.date_time, p.post_name,p.image, p.province) "
                        ...
                        + "ORDER by p.date_time DESC")
        List<RelatedPostResponse> getRelatedPost(@Param("postId") Integer postId,
                        @Param("provinceId") Province provinceId);

        @Query(value = "SELECT p2.id_post, province_name, p.id_province ,p2.image , COUNT(l.id_post) AS total_likes "
                        + "FROM posts p2, provinces p, likes l "
                        + "where p2.id_post = l.id_post "
                        + "and p2.id_province = p.id_province "
                        + "GROUP BY province_name, p.id_province , p2.id_post ,p2.image "
                        + "ORDER BY total_likes DESC "
                        + "LIMIT 3", nativeQuery = true)
        List<Object[]> getPostAndProvince();

        @Query("SELECT new com.example.TravelinVServer.Responese.GoToResponse (p.province_name, p.id_province) "
