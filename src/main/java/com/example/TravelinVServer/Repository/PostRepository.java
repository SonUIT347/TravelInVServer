...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    @Query("SELECT new com.example.TravelinVServer.Responese.PostResponse(p.id_post, p.date_time, p.post_name, p.image, p.demoDescription, p.status, p.user.id_user, p.province.id_province) "
            + "FROM posts p "
            + "ORDER BY ("
            + "CASE p.status "
            + "WHEN 'Pending' THEN 1 "
